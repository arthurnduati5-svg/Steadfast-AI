...
export async function emotional_decoder(args) {
    ...
    if (insults.some(w => raw.includes(w)))
        return { emotion: "angry_insult", triggers: [raw], suggestedLanguageMode: "english" };
...
export async function math_validate_answer(args) {
    ...
    if (!qClean)
        return { isCorrect: false, explanation: "I couldn't read the numbers." };
    try {
        ...
        if (studentVal === null)
            return { isCorrect: false, explanation: "Please write the number." };
