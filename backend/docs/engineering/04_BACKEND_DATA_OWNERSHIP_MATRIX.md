...
ChatMessage | src/routes/ai.ts | src/routes/ai/ai-chat.routes.ts, src/services/aiService.ts | DUPLICATE_WRITER_CANDIDATE | PRISMA_WRITE_ACCESS+R8A_STRUCTURAL | modelWriterGroups:ChatMessage; Multiple distinct production writer files without a proven coordination boundary (filename co-occurrence is candidate signal only). CANONICAL MUTATION PATH from 10 production WRITE access row(s): src/routes/ai.ts lines 4791, 5077, 5123, 5551, 5670 ops create/update (PRISMA_WRITE_ACCESS).
...
ResultReleasePacketRecord | src/domains/assessment/result-release/repositories/prismaResultReleaseRepositories.ts | — | CLEAR | PRISMA_WRITE_ACCESS+R8A_STRUCTURAL | modelWriterGroups:ResultReleasePacketRecord; Single production writer file. CANONICAL MUTATION PATH from 6 production WRITE access row(s): src/domains/assessment/result-release/repositories/prismaResultReleaseRepositories.ts lines 234, 287, 292, 297, 305 +1 more ops create/update (PRISMA_WRITE_ACCESS).
