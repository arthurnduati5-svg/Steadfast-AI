...
export class RecoveryProgressSafetyService {
  ...
  assertNoLiveProgressMutation(payload: Record<string, unknown>): { allowed: boolean; reasonCode: string; safeMessage: string } {
    return this.checkFields(payload, ['scoreMutationPayload', 'masteryMutationPayload', 'resultOverwritePayload', 'liveProgressUpdatePayload'], 'LIVE_PROGRESS_MUTATION', 'Live progress mutation detected');
  }

  assertObservationUsesReferencesOnly(payload: Record<string, unknown>): { allowed: boolean; reasonCode: string; safeMessage: string } {
    const checks = [
      ...
      this.assertNoLiveProgressMutation(payload),
    ];
    for (const c of checks) {
      if (!c.allowed) return c;
    }
    return { allowed: true, reasonCode: 'SAFE', safeMessage: 'Observation uses references only' };
  }

  assertParentNoteDraftSafe(payload: Record<string, unknown>): { allowed: boolean; reasonCode: string; safeMessage: string } {
    const checks = [
      this.assertNoAnswerKeyLeakage(payload),
      this.assertNoRubricLeakage(payload),
      this.assertNoRawStudentAnswerLeakage(payload),
    ...
    return { allowed: true, reasonCode: 'SAFE', safeMessage: 'Parent note draft is safe' };
  }

  assertReflectionDraftSafe(payload: Record<string, unknown>): { allowed: boolean; reasonCode: string; safeMessage: string } {
    ...
    return { allowed: true, reasonCode: 'SAFE', safeMessage: 'Reflection draft is safe' };
  }

  assertEvaluationReferencesSafe(payload: Record<string, unknown>): { allowed: boolean; reasonCode: string; safeMessage: string } {
    const checks = [
      this.assertNoAnswerKeyLeakage(payload),
      this.assertNoRubricLeakage(payload),
      this.assertNoRawStudentAnswerLeakage(payload),
      this.assertNoUnreleasedGradeLeakage(payload),
      this.assertNoScoreMutation(payload),
      this.assertNoMasteryMutation(payload),
      this.assertNoLiveProgressMutation(payload),
    ];
    for (const c of checks) {
      if (!c.allowed) return c;
    }
    return { allowed: true, reasonCode: 'SAFE', safeMessage: 'Evaluation references are safe' };
  }

  assertMockOnlyProgressOperation(mode: string): { allowed: boolean; reasonCode: string; safeMessage: string } {
    const allowedModes = ['mock_observation_only', 'future_progress_observation', 'teacher_review_only', 'metadata_only'];
    const evalAllowedModes = ['mock_evaluation_only', 'future_checkpoint_evaluation', 'teacher_review_only', 'metadata_only'];
    if (allowedModes.includes(mode) || evalAllowedModes.includes(mode)) {
      return { allowed: true, reasonCode: 'SAFE', safeMessage: `Mock progress operation allowed for mode ${mode}` };
    }
    return { allowed: false, reasonCode: 'LIVE_ACTION_BLOCKED', safeMessage: `Live progress mode ${mode} is blocked. Only mock/future/teacher-review modes are allowed.` };
  }
