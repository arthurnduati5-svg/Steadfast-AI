export type ExamModeRole = 'student' | 'teacher' | 'admin' | 'system';

export interface ExamModeAccessRequest {
  requesterSchoolId: string;
  requesterStudentId?: string;
  requesterRole: ExamModeRole;
  targetSchoolId: string;
  targetStudentId: string;
  targetTutorLearnerId?: string;
}

export interface ExamModeAccessResult {
  allowed: boolean;
  reasonCodes: string[];
}

export function checkExamModeAccess(req: ExamModeAccessRequest): ExamModeAccessResult {
