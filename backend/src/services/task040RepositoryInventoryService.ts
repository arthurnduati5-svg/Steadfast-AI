...
export function buildRepositoryInventory(): Task040RepositoryInventoryEntry[] {
  const dir = path.resolve(process.cwd(), 'backend/src/repositories');
  const entries: Task040RepositoryInventoryEntry[] = [];
  try {
    ...
    for (const file of files) {
      ...
      entries.push({
        path: `backend/src/repositories/${file}`,
        taskOwner,
        category: 'repository',
        ...
        notes: isTask040 ? 'Task 040 freeze repository' : `Backend repository from task ${taskOwner}`,
      });
    }
  } catch { }
  return entries;
...
export function getRepositoryInventory(): Task040RepositoryInventoryEntry[] {
  const existing = task040Repository.getRepositoryInventory();
