...
export function buildServiceInventory(): Task040ServiceInventoryEntry[] {
  const dir = path.resolve(process.cwd(), 'backend/src/services');
  const entries: Task040ServiceInventoryEntry[] = [];
  try {
    ...
    for (const file of files) {
      ...
      entries.push({
        path: `backend/src/services/${file}`,
        taskOwner,
        category: 'service',
        ...
        notes: isTask040 ? 'Task 040 freeze service' : `Backend service from task ${taskOwner}`,
      });
    }
  } catch { }
  return entries;
...
export function getServiceInventory(): Task040ServiceInventoryEntry[] {
  const existing = task040Repository.getServiceInventory();
