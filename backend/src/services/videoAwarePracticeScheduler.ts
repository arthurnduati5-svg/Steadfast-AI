// ─────────────────────────────────────────────────────────────
// Steadfast AI — Video-Aware Practice Scheduler v1
...
const INTERVAL_FIRST_REVIEW = 1;       // 1 day
const INTERVAL_SECOND_REVIEW = 3;      // 3 days
const INTERVAL_THIRD_REVIEW = 7;       // 7 days
const INTERVAL_IMMEDIATE_RETEACH = 0;  // Same day

/**
 * Calculate the due date for a spaced review based on the number of
 * successful spaced reviews already completed for this topic.
 */
...
/**
 * Calculate the due date for an immediate reteach or same-day review.
 */
...
/**
 * Check if a scheduled review is due (current date >= dueAt).
 */
 ...
 */
export function determineReviewSchedule(
  ...
  // Incorrect or needs_review: immediate/same-day reteach
  ...
  // Correct: count successful reviews from session history
  if (answerStatus === 'correct') {
    // Count completed sessions for the same topic
    const successfulCount = session.skillIds.length > 0 ? 1 : 0; // Simplified v1
  ...
  // Partially correct: review sooner
...
export function generateReviewPrompt(
  ...
  if (isDue) {
    return 'Your review is due now. Let us revisit this topic to reinforce your learning.';
  }
  ...
  try {
    ...
    if (diffHours <= 0) {
      return 'Your review is ready. Let us practice to reinforce your understanding.';
    }
    if (diffHours < 24) {
