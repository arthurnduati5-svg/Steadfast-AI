...
describe('Task 040 continuity from task 026', () => {
  it('includes task 026 in accepted task IDs', () => {
    expect(TASK040_ACCEPTED_TASK_IDS.includes('026')).toBe(true);
  });

  it('has route file for task 026', () => {
    const dir = path.resolve(__dirname, '..', 'routes');
    const files = fs.readdirSync(dir);
    expect(files.some(f => f.includes('task026'))).toBe(true);
  });
});
