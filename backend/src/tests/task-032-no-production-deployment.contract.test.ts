    ...
    const gather = (dir: string, acc: string[]) => {
      ...
      for (const e of entries) {
        ...
        if (e.isDirectory() && !e.name.startsWith('node_modules') && !e.name.startsWith('.git') && !e.name.startsWith('.planning')) {
          gather(full, acc);
        } else if ((e.name.endsWith('.ts') || e.name.endsWith('.js') || e.name.endsWith('.cjs')) && (e.name.includes('task-032') || e.name.includes('task032')) && !e.name.endsWith('.test.ts') && !e.name.includes('Contracts') && !e.name.includes('Service') && !e.name.includes('Repository') && !e.name.includes('Validation') && !e.name.includes('Routes')) {
          acc.push(full);
        }
      }
    };
