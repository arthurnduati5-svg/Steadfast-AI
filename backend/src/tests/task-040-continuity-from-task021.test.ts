...
describe('Task 040 continuity from task 021', () => {
  it('includes task 021 in accepted task IDs', () => {
    expect(TASK040_ACCEPTED_TASK_IDS.includes('021')).toBe(true);
  });

  it('has route file for task 021', () => {
    const dir = path.resolve(__dirname, '..', 'routes');
    const files = fs.readdirSync(dir);
    expect(files.some(f => f.includes('task021'))).toBe(true);
  });
});
