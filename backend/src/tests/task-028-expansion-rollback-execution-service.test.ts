...
describe('Task 028 Expansion Rollback Execution Service', () => {
  ...
  beforeEach(async () => {
    ...
    const run = await task028ExpansionExecutionRepository.createExecutionRun({
      ...
      status: 'stage_1_active', safeSummary: 'Active expansion for rollback test',
    });
  ...
  it('should preserve learning evidence with dataDeleted=false and auditPreserved=true', async () => {
  ...
  it('should record rollback metadata', async () => {
    const result = await executeRollback(executionRunId, 'operator', 'op-hash', 'Privacy breach');
    expect(result.dataDestructivelyDeleted).toBe(false);
    expect(result.auditPreserved).toBe(true);
