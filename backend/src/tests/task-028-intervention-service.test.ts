...
describe('Task 028 Intervention Service', () => {
  ...
  beforeEach(async () => {
    ...
    const run = await task028ExpansionExecutionRepository.createExecutionRun({
      ...
      safeSummary: 'Active expansion for intervention test',
    });
    ...
    await task028ExpansionExecutionRepository.createExecutionStage({
      ...
      plannedTeacherCount: 2,
      safeSummary: 'Stage 1 active for intervention test',
      allowedClassIds: ['class-1'],
      allowedSubjectIds: ['subject-1'],
      allowedCurriculumScopes: ['scope-1'],
    });
  });
  ...
  it('should request an intervention', async () => {
    const result = await requestIntervention(
      ...
      'teacher',
      'teacher-hash-1',
      'Teacher requested pause for review',
    );
    expect(result.ok).toBe(true);
    expect(result.interventionId).toBeTruthy();
  });
  ...
  it('should complete an intervention', async () => {
    const request = await requestIntervention(
      executionRunId,
      'pause_execution',
      'teacher',
      'teacher-hash-1',
      'Teacher requested pause',
    );
    const result = await completeIntervention(
      request.interventionId!,
      'admin',
      'admin-hash',
      'Intervention completed by admin',
      executionRunId,
      'pp-1',
      'school-1',
    );
