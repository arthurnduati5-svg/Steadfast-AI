...
describe('Task 040 continuity from task 027', () => {
  it('includes task 027 in accepted task IDs', () => {
    expect(TASK040_ACCEPTED_TASK_IDS.includes('027')).toBe(true);
  });

  it('has route file for task 027', () => {
    const dir = path.resolve(__dirname, '..', 'routes');
    const files = fs.readdirSync(dir);
    expect(files.some(f => f.includes('task027'))).toBe(true);
  });
});
