...
describe('Task 028 Monitoring Event Service', () => {
  ...
  it('should record a valid expansion monitoring event', async () => {
    ...
    const result = await recordExpansionMonitoringEvent({
      ...
      safeSummary: 'Preflight checks passed for expansion',
      requestId: 'req-001',
  ...
  it('should reject event with forbidden content in safeSummary', async () => {
    ...
    const result = await recordExpansionMonitoringEvent({
      ...
      eventType: 'expansion_preflight_passed',
      eventStatus: 'completed',
      safeSummary: 'This contains raw student chat content',
    });
    expect(result.ok).toBe(false);
