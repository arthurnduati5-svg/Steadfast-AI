import { describe, it, expect } from 'vitest';
import task030Routes from '../routes/task030ControlledStagingRehearsalRoutes';

describe('Task 030 - Routes Synthetic Fixture Contract', () => {
  it('should have synthetic-school fixture route', () => {
    const stack = (task030Routes as any).stack;
    const has = stack.some((layer: any) =>
      layer.route && layer.route.path && layer.route.path.includes('fixtures/synthetic-school')
    );
    expect(has).toBe(true);
  });

  it('should be POST method', () => {
    const stack = (task030Routes as any).stack;
    const route = stack.find((layer: any) =>
      layer.route && layer.route.path && layer.route.path.includes('fixtures/synthetic-school')
    );
    expect(route.route.methods).toHaveProperty('post');
  });

  it('should have role-token-matrix route', () => {
    const stack = (task030Routes as any).stack;
    const has = stack.some((layer: any) =>
      layer.route && layer.route.path && layer.route.path.includes('role-token-matrix')
    );
    expect(has).toBe(true);
  });

  it('should be POST method for role-token-matrix', () => {
    const stack = (task030Routes as any).stack;
    const route = stack.find((layer: any) =>
      layer.route && layer.route.path && layer.route.path.includes('role-token-matrix')
    );
    expect(route.route.methods).toHaveProperty('post');
  });

  it('should have runs POST route', () => {
    const stack = (task030Routes as any).stack;
    const route = stack.find((layer: any) =>
      layer.route && layer.route.path && layer.route.path === '/task030/controlled-staging-rehearsal/runs' && layer.route.methods.post
    );
    expect(route).toBeDefined();
  });

  it('should have runs GET route with runId param', () => {
    const stack = (task030Routes as any).stack;
    const route = stack.find((layer: any) =>
      layer.route && layer.route.path && layer.route.path.includes('runs/:runId') && layer.route.methods.get
    );
    expect(route).toBeDefined();
  });
});
