...
import { getCapabilityPrompt } from './capabilities';

export type SupportedLanguage = 'english' | 'swahili' | 'arabic' | 'english_sw' | 'arabic_english';
export type DeliveryLayerContext = {
  voiceMode?: boolean;
  ...
  advancedStudentHint?: boolean;
  strugglingHint?: boolean;
};
...
const EXAM_MODE_HINT_REGEX =
  /\b(exam|finals?|midterm|mock|revision|past\s*paper|mark\s*scheme|marks?\s*allocation|time\s*management|kcse|kcpe|assessment|test\s*prep)\b/i;
const YOUNG_GRADE_HINT_REGEX =
  /\b(kindergarten|kinder|nursery|pre[-\s]?primary|pp1|pp2|lower\s*primary|grade\s*[1-3]\b|class\s*[1-3]\b)\b/i;
const TEEN_GRADE_HINT_REGEX =
  /\b(secondary|lower\s*secondary|upper\s*secondary|high\s*school|form\s*[1-4]\b|grade\s*(?:7|8|9|10|11|12)\b|class\s*(?:7|8|9|10|11|12)\b)\b/i;
const STRUGGLE_HINT_REGEX =
  /\b(i\s+do\s+not\s+understand|i\s+don't\s+understand|not\s+clear|confused|stuck|hard\s+for\s+me|too\s+hard|lost|please\s+explain\s+again|can\s+you\s+repeat|slow\s+down)\b/i;
const ADVANCED_HINT_REGEX =
  /\b(challenge\s+me|harder|advanced|deeper|skip\s+the\s+basics|assume\s+i\s+know|i\s+already\s+know|push\s+me|prove\s+it)\b/i;
const PROCEDURAL_HINT_REGEX =
  /\b(solve|calculate|compute|work\s+out|show\s+steps|step\s+by\s+step|procedure|algorithm|derive|simplify)\b/i;
...
function inferMaturityStage(gradeLevel?: string | null, userText?: string): 'young_kids' | 'teen' | null {
  const grade = String(gradeLevel || '').toLowerCase();
  const text = String(userText || '').toLowerCase();
  const combined = `${grade} ${text}`.trim();

  if (!combined) return null;
  if (YOUNG_GRADE_HINT_REGEX.test(combined)) return 'young_kids';
  if (TEEN_GRADE_HINT_REGEX.test(combined)) return 'teen';

  const gradeNum = parseGradeNumber(combined);
  if (gradeNum !== null && gradeNum <= 3) return 'young_kids';
  if (gradeNum !== null && gradeNum >= 7) return 'teen';
  return null;
}

function detectExamMode(userText?: string): boolean {
  return EXAM_MODE_HINT_REGEX.test(String(userText || ''));
}

function detectStudentStruggle(userText?: string, validationAttemptCount?: number, explicitHint?: boolean): boolean {
  if (explicitHint) return true;
  if (Number(validationAttemptCount || 0) >= 2) return true;
  return STRUGGLE_HINT_REGEX.test(String(userText || ''));
}

function detectAdvancedStudent(userText?: string, explicitHint?: boolean): boolean {
  if (explicitHint) return true;
  return ADVANCED_HINT_REGEX.test(String(userText || ''));
}

function detectProceduralMode(userText?: string, explicitHint?: boolean): boolean {
  if (explicitHint) return true;
  return PROCEDURAL_HINT_REGEX.test(String(userText || ''));
}

export function getAdaptiveDeliveryLayers(context?: DeliveryLayerContext): string {
  const ctx = context || {};
  const blocks: string[] = [];
  ...
  if (ctx.voiceMode) blocks.push(VOICE_MODE_SPECIFIC_LAYER);
  if (stage === 'young_kids') blocks.push(YOUNG_KIDS_LAYER);
  if (stage === 'teen') blocks.push(TEEN_ACADEMIC_LAYER);
  if (examModeActive) blocks.push(EXAM_COACH_LAYER);
...
export function getMasterBehavioralOrchestrationLayer(context?: DeliveryLayerContext): string {
  const ctx = context || {};
  const stage = inferMaturityStage(ctx.gradeLevel, ctx.userText);
  const examModeActive = Boolean(ctx.examMode) || detectExamMode(ctx.userText);
...
const ENGLISH_PROFILE: LanguageProfile = {
  ...
  culturalContext:
    'Use examples from daily Kenyan life (markets, farming, sports). Keep language Muslim-school appropriate and respectful.',
  rules: [
  ...
  failureStrategy:
    'If a tool/video fails, do not announce tool failure. Seamlessly continue teaching the concept directly.'
};

const ARABIC_PROFILE: LanguageProfile = {
  identity:
    '\u0623\u0646\u062a \u00ab\u0645\u0639\u0644\u0645 \u0633\u062a\u064a\u062f\u0641\u0627\u0633\u062a\u00bb\u060c \u0645\u0639\u0644\u0645 \u062d\u0643\u064a\u0645 \u0648\u0635\u0628\u0648\u0631 \u064a\u0639\u0644\u0651\u0645 \u0627\u0644\u0637\u0644\u0627\u0628 \u0628\u0623\u0633\u0644\u0648\u0628 \u0648\u0627\u0636\u062d.',
  tone:
    '\u0647\u0627\u062f\u0626\u060c \u0645\u0634\u062c\u0639\u060c \u0648\u0648\u0627\u062b\u0642. \u0644\u063a\u0629 \u062a\u0631\u0628\u0648\u064a\u0629 \u0631\u0635\u064a\u0646\u0629 \u063a\u064a\u0631 \u0631\u0648\u0628\u0648\u062a\u064a\u0629.',
  culturalContext:
    '\u0627\u0633\u062a\u062e\u062f\u0645 \u0627\u0644\u0639\u0631\u0628\u064a\u0629 \u0627\u0644\u0641\u0635\u062d\u0649 \u0627\u0644\u0645\u0628\u0633\u0637\u0629\u060c \u0645\u0639 \u0623\u062f\u0628 \u0645\u0646\u0627\u0633\u0628 \u0644\u0645\u062f\u0631\u0633\u0629 \u0645\u0633\u0644\u0645\u0629.',
  rules: [
  ...
  failureStrategy:
    '\u0625\u0630\u0627 \u062a\u0639\u0630\u0631\u062a \u0623\u062f\u0627\u0629 \u062e\u0627\u0631\u062c\u064a\u0629\u060c \u0648\u0627\u0635\u0644 \u0627\u0644\u0634\u0631\u062d \u0645\u0628\u0627\u0634\u0631\u0629 \u0628\u062f\u0648\u0646 \u0625\u0638\u0647\u0627\u0631 \u0631\u0633\u0627\u0626\u0644 \u0641\u0634\u0644.'
};
...
const SWAHILI_PROFILE: LanguageProfile = {
  ...
  failureStrategy:
    'Kama tool imefeli, usionyeshe kosa kwa mwanafunzi. Endelea kufundisha moja kwa moja kwa kujiamini.'
};
...
const ARABIC_ENGLISH_PROFILE: LanguageProfile = {
...
export function getLanguageGovernance(lang: string, interests: string[]): string {
  ...
  if (normalizedLang.includes('arabic_english') || normalizedLang.includes('ar_en')) profile = ARABIC_ENGLISH_PROFILE;
  else if (normalizedLang.includes('arab')) profile = ARABIC_PROFILE;
...
export function getDeliveryRefinementLayer(): string {
