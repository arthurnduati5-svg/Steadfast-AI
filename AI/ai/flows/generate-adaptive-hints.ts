...
const GenerateAdaptiveHintOutputSchema = z.object({
  ...
  hintLevel: z.enum(['H0', 'H1', 'H2', 'H3', 'GUIDED_SOLVE', 'REFUSAL']).describe('The level of the hint provided.'),
...
const PromptOutputSchema = z.object({
  generatedHint: z.string(),
  determinedNextHintIndex: z.number(),
  determinedHintLevel: z.enum(['H0', 'H1', 'H2', 'H3', 'GUIDED_SOLVE', 'REFUSAL']),
  suggestedActions: z.array(z.string()),
  responseTone: z.enum(['neutral', 'supportive', 'celebratory', 'deescalate']),
  determinedMode: z.enum(['daily_objective', 'chat', 'history']),
  determinedKcTags: z.array(z.string()),
});
...
function levelFromIndex(index: number): 'H0' | 'H1' | 'H2' | 'H3' {
  if (index <= 0) return 'H0';
  if (index === 1) return 'H1';
  if (index === 2) return 'H2';
  return 'H3';
}

function detectRefusal(progress: string): boolean {
  return /\b(just give.*answer|final answer|solve it for me|just tell me answer)\b/i.test(progress);
}

function detectGuidedSolveTrigger(input: GenerateAdaptiveHintInput): boolean {
  ...
  const masteryLow =
    typeof input.masteryLevel === 'number' &&
...
function fallbackHint(input: GenerateAdaptiveHintInput): GenerateAdaptiveHintOutput {
  ...
  if (refusal) {
    return {
      hint: ensureSingleQuestionAtEnd("I cannot give the final answer directly, but I can guide you through the method."),
  ...
  if (guided) {
    ...
    return {
      hint: ensureSingleQuestionAtEnd(`Guided-Solve Mode. ${normalizeTutorText(guidedText)}`),
      nextHintIndex: next,
  ...
  return {
    hint: ensureSingleQuestionAtEnd(normalizeTutorText(hintText)),
...
function mapPromptOutput(output: PromptOutput): GenerateAdaptiveHintOutput {
  return {
    hint: ensureSingleQuestionAtEnd(normalizeTutorText(output.generatedHint)),
...
const generateAdaptiveHintFlow = async (input: GenerateAdaptiveHintInput): Promise<GenerateAdaptiveHintOutput> => {
  ...
  const mode = input.isDailyObjectiveMode ? 'daily_objective' : 'chat';
  const basePrompt = buildUnifiedTutorPrompt({ mode: 'hints', language: 'english' });
...
`;
  ...
  try {
    ...
    if (!content) return fallbackHint(input);
