...
export type TurnSecondaryAction = 'practice' | 'research' | 'video_lookup';

export type LearningEvidenceMode =
  | 'topic_context'
  | 'artifact_context'
  | 'video_context'
  | 'revision_context'
...
export type LearningTurnPlan = {
  ...
  researchReason:
    ...
    | 'source_or_verification_request'
    | 'time_sensitive_topic'
    | 'guided_web_mode'
    | 'explicit_no_web_request'
    | 'meta_mode_question';
...
const VALID_EVIDENCE_MODES: LearningEvidenceMode[] = [
  'topic_context',
  'artifact_context',
  'video_context',
  'revision_context',
...
const VALID_EVIDENCE_MODE_SET = new Set<string>(VALID_EVIDENCE_MODES);
...
function isWebModeMetaQuery(query: string): boolean {
  ...
  return (
    /\b(are you|did you|why are you|how are you|can you|will you)\b[^?.!]{0,120}\b(search|searching|look up|web|online|source|sources|citation|verify)\b/.test(
      lower
    ) ||
...
function hasExplicitNoWebSignal(query: string): boolean {
  ...
  if (!lower) return false;
  return (
    /\b(no web|don't search|do not search|without searching|no internet|without internet|from your knowledge|from context only)\b/.test(
      lower
    ) || /\bjust explain|no lookup|no research\b/.test(lower)
  );
}

function hasRecentAssistantContext(turns?: RecentTurn[]): boolean {
  return Array.isArray(turns)
    ? turns.some((turn) => turn?.role === 'assistant' && String(turn.content || '').trim().length > 0)
    : false;
}

function isLikelyFollowUpQuery(query: string, recentTurns?: RecentTurn[]): boolean {
  const lower = query.trim().toLowerCase();
  if (!lower) return false;
  const words = lower.split(/\s+/).filter(Boolean);
  const hasAssistantContext = hasRecentAssistantContext(recentTurns);
  if (words.length > 28 && !hasAssistantContext) return false;

  return (
    /\b(this|that|it|those|these|above|earlier|previous|same|part|point|line|step|answer)\b/.test(
      lower
    ) ||
    /\b(you said|your answer|that answer|this answer|explain that|explain this|continue from that)\b/.test(
      lower
    ) ||
    /^(and|so|then|but|okay|ok|continue)\b/.test(lower) ||
    (hasAssistantContext &&
      /^(why|how|what do you mean|can you explain|explain again|break (it|that) down|summari[sz]e that|what about|which part)\b/.test(
        lower
      ))
  );
}

function hasExplicitTopicShift(query: string): boolean {
  return /\b(new topic|another topic|different topic|change topic|switch topic|unrelated|instead|let'?s talk about|move to)\b/i.test(
    query
  );
}

function wantsExplanation(query: string): boolean {
  const lower = query.trim().toLowerCase();
  if (!lower) return false;
  return (
    /\b(explain|teach|help me understand|walk me through|analyse|analyze|review|break down|what does this mean)\b/.test(
      lower
    ) || /\b(why|how does|how do)\b/.test(lower)
  );
}

function wantsSummary(query: string): boolean {
  return /\b(summarise|summarize|give me a summary|short recap|recap)\b/i.test(query);
}

function wantsCompare(query: string): boolean {
  return /\b(compare|difference between|versus|vs\b|how is .* different)\b/i.test(query);
}

function wantsHint(query: string): boolean {
  return /\b(hint|small hint|nudge|clue)\b/i.test(query);
}

function wantsPractice(query: string): boolean {
  return /\b(test me|quiz|practice|question me|give me a question|checkpoint|similar question)\b/i.test(
    query
  );
}
...
function wantsVerification(query: string): boolean {
  return /\b(verify|verification|fact check|is this true|is that true|is this correct|check if this is correct|confirm)\b/i.test(
    query
  );
}

function wantsReverify(query: string): boolean {
  return /\b(are you sure|double check|double-check|check again|re-verify|reverify|verify again)\b/i.test(
    query
  );
}

function wantsSourceDiversity(query: string): boolean {
  return /\b(another source|different source|more sources|find another source|source from somewhere else)\b/i.test(
    query
  );
}

function wantsLatestInfo(query: string): boolean {
  return /\b(latest|current|today|recent|updated|update|news|this week|this month|this year|now)\b/i.test(
    query
  );
}

function wantsExplicitResearch(query: string, forceWebSearch?: boolean): boolean {
  if (forceWebSearch) return true;
  return /\b(research|look up|search online|search the web|use the web|browse online|find reliable sources)\b/i.test(
    query
  );
}

function isCurrentVideoFollowUp(query: string): boolean {
  const lower = query.trim().toLowerCase();
  if (!lower) return false;
  if (!/\b(video|youtube|watch|transcript|clip)\b/.test(lower)) return false;
  return /\b(content|contents|about|summary|summari[sz]e|explain|cover|covers|teach|what is in|what's in|what does the video say|that part)\b/.test(
    lower
  );
}

function isCurrentFileFollowUp(query: string): boolean {
  const lower = query.trim().toLowerCase();
  if (!lower) return false;
  return /\b(file|image|document|pdf|attachment|attached|uploaded|poster|design|page|worksheet|notes|diagram|graph|table)\b/.test(
    lower
  );
}
...
function isRevisionFollowUp(query: string): boolean {
  const lower = query.trim().toLowerCase();
  if (!lower) return false;
  return /\b(revision|saved note|saved item|saved card|this note|this revision item|revise this|quiz me on this saved)\b/.test(
    lower
  );
}

function isDialogueContinuation(query: string): boolean {
...
function isDefinitionRequest(query: string): boolean {
  return /\b(what is|define|definition of)\b/i.test(query);
}

function inferTopicStability(query: string, lastTopic?: string): TopicStability {
  const sample = `${lastTopic || ''} ${query}`.toLowerCase();
  if (
    /\b(latest|current|today|recent|updated|news|breaking|price|stock|rate|exchange rate|inflation|election|president|prime minister|ceo|version|release|live|currently)\b/.test(
      sample
    ) || /\b(202[4-9]|203[0-9])\b/.test(sample)
  ) {
    return 'time_sensitive';
  }

  if (
    /\b(policy|regulation|rules|guideline|medical advice|health recommendation|admissions|application deadline|technology|software|library|framework)\b/.test(
      sample
    )
  ) {
    return 'semi_stable';
...
function coercePrimaryAction(raw: string): TurnPrimaryAction {
  const normalized = normalizeIntentText(raw);
  if (normalized.includes('current_video')) return 'current_video';
  if (normalized.includes('video_lookup') || normalized === 'video') return 'video_lookup';
  if (normalized.includes('practice')) return 'practice';
  if (normalized.includes('research') || normalized.includes('lookup')) return 'research';
...
function coerceSecondaryAction(raw: string | undefined): TurnSecondaryAction | undefined {
  const normalized = normalizeIntentText(raw || '');
  if (!normalized) return undefined;
  if (normalized.includes('video')) return 'video_lookup';
  if (normalized.includes('practice')) return 'practice';
  if (normalized.includes('research') || normalized.includes('lookup')) return 'research';
  return undefined;
}

function coerceEvidenceMode(raw: string, fallback: LearningEvidenceMode): LearningEvidenceMode {
  const normalized = normalizeIntentText(raw);
  return VALID_EVIDENCE_MODE_SET.has(normalized) ? (normalized as LearningEvidenceMode) : fallback;
}

function coerceIntent(raw: string, query: string, lastTopic?: string): ResearchIntent {
  const normalized = normalizeIntentText(raw);
...
function resolveContextTarget(
  ...
  activeTopic: string,
  likelyFollowUp: boolean,
  options?: LearningTurnPlanOptions
): LearningTurnPlan['currentContextTarget'] {
...
function isResearchWorthwhile(
  query: string,
  contextTarget: LearningTurnPlan['currentContextTarget'],
  likelyFollowUp: boolean
): boolean {
  const lower = query.trim().toLowerCase();
  if (!lower) return false;
  if (hasExplicitNoWebSignal(lower) || isWebModeMetaQuery(lower)) return false;
  if (contextTarget !== 'none' && likelyFollowUp) return false;
  if (
    /^(why|how|what do you mean|can you explain|explain again|break it down|summari[sz]e that|what about this line|quiz me|similar question)\b/.test(
      lower
    )
  ) {
    return false;
...
function buildHeuristicTurnPlan(
  query: string,
  lastTopic?: string,
  options?: LearningTurnPlanOptions
): LearningTurnPlan {
  ...
  const reverify = wantsReverify(lower);
  const sourceDiversity = wantsSourceDiversity(lower);
  ...
  let evidenceMode: LearningEvidenceMode =
    currentContextTarget === 'video'
      ? 'video_context'
  ...
  let researchReason: LearningTurnPlan['researchReason'] = 'context_first';
  ...
  if (noWeb) {
    ...
    (verification || reverify || sourceDiversity || sources) &&
    ...
    (latestInfo || topicStability === 'time_sensitive') &&
    isResearchWorthwhile(lower, currentContextTarget, likelyFollowUp)
  ) {
    researchRequired = true;
  ...
  let intent: ResearchIntent = 'clarification';
  let primaryAction: TurnPrimaryAction = 'clarification';
  let secondaryAction: TurnSecondaryAction | undefined;
  ...
  return {
    ...
    wantsVideo: video,
    wantsWebResearch: explicitResearch || latestInfo || sources || verification || reverify || sourceDiversity,
    wantsSources: sources,
    wantsVerification: verification || reverify,
...
export function heuristicResearchIntent(query: string, lastTopic?: string): ResearchIntent {
  return buildHeuristicTurnPlan(query, lastTopic).intent;
}

export async function detectResearchIntent(query: string, lastTopic?: string): Promise<ResearchIntent> {
  const heuristicPlan = buildHeuristicTurnPlan(query, lastTopic);
  if (
    heuristicPlan.intentConfidence >= 0.84 ||
...
`;
  ...
  try {
    ...
    const modelIntent = (response.output as { intent?: unknown } | null)?.intent;
...
export async function detectLearningTurnPlan(
  query: string,
  lastTopic?: string,
  options?: LearningTurnPlanOptions
  ...
  if (
    heuristicPlan.intentConfidence >= 0.86 ||
...
`;
  ...
  try {
    ...
    const primaryAction =
      typeof output.primaryAction === 'string'
        ? coercePrimaryAction(output.primaryAction)
        : heuristicPlan.primaryAction;
    const secondaryAction =
      typeof output.secondaryAction === 'string' && output.secondaryAction.toLowerCase() !== 'none'
        ? coerceSecondaryAction(output.secondaryAction)
        : heuristicPlan.secondaryAction;

    const currentContextTarget =
      output.currentContextTarget === 'video' ||
    ...
    const researchReason =
      output.researchReason === 'context_first' ||
    ...
    const intentConfidence = Math.max(
      ...
      Math.min(
        1,
        typeof output.intentConfidence === 'number'
    ...
    return {
      ...
      secondaryAction,
      explicitTopicShift:
        typeof output.explicitTopicShift === 'boolean'
          ? output.explicitTopicShift
          : heuristicPlan.explicitTopicShift,
      likelyFollowUp:
        typeof output.likelyFollowUp === 'boolean'
          ? output.likelyFollowUp
          : heuristicPlan.likelyFollowUp,
      wantsExplanation:
        typeof output.wantsExplanation === 'boolean'
          ? output.wantsExplanation
          : heuristicPlan.wantsExplanation,
      wantsPractice:
        typeof output.wantsPractice === 'boolean'
          ? output.wantsPractice
          : heuristicPlan.wantsPractice,
      wantsVideo:
        typeof output.wantsVideo === 'boolean' ? output.wantsVideo : heuristicPlan.wantsVideo,
      wantsWebResearch:
        typeof output.wantsWebResearch === 'boolean'
          ? output.wantsWebResearch
          : heuristicPlan.wantsWebResearch,
      wantsSources:
        typeof output.wantsSources === 'boolean' ? output.wantsSources : heuristicPlan.wantsSources,
      wantsVerification:
        typeof output.wantsVerification === 'boolean'
          ? output.wantsVerification
          : heuristicPlan.wantsVerification,
      wantsLatestInfo:
        typeof output.wantsLatestInfo === 'boolean'
      ...
      evidenceMode,
      researchRequired:
        typeof output.researchRequired === 'boolean'
